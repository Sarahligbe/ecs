...
resource "aws_appautoscaling_policy" "ecs_cpu_policy" {
  name               = "${var.ecs_cluster_name}-cpu-policy"
...
resource "aws_appautoscaling_policy" "ecs_memory_policy" {
  name               = "${var.ecs_cluster_name}-memory-policy"
  policy_type        = "TargetTrackingScaling"
  resource_id        = aws_appautoscaling_target.ecs_target.resource_id
  scalable_dimension = aws_appautoscaling_target.ecs_target.scalable_dimension
  service_namespace  = aws_appautoscaling_target.ecs_target.service_namespace
...
resource "aws_cloudwatch_metric_alarm" "request_count_alarm" {
  alarm_name          = "${var.ecs_cluster_name}-high-request-count"
  ...
  statistic          = "Sum"
  threshold          = "1000"
  alarm_description  = "Request count threshold exceeded"
  alarm_actions      = [aws_appautoscaling_policy.ecs_request_policy.arn]
